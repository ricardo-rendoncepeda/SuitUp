...
class FrontView: UIView {
  ...
  init(suit: String, rank: String) {
    ...
    suitView.setNeedsDisplay()
    
    // Label
    let rankLabel = UILabel(frame: CGRectMake(0, 0, 100, 100))
    rankLabel.text = rank
    rankLabel.textColor = secondaryColor
    rankLabel.font = labelFont
    rankLabel.sizeToFit()
